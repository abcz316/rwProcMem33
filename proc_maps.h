...
static inline bool check_proc_map_can_read(struct pid* proc_pid_struct, size_t proc_virt_addr, size_t size)
{
	struct task_struct *task = get_pid_task(proc_pid_struct, PIDTYPE_PID);
	struct mm_struct *mm;
...
static inline bool check_proc_map_can_write(struct pid* proc_pid_struct, size_t proc_virt_addr, size_t size)
{
	struct task_struct *task = get_pid_task(proc_pid_struct, PIDTYPE_PID);
	struct mm_struct *mm;
	struct vm_area_struct *vma;
...
//Update: get_proc_maps_list_to_kernel: fs\proc\task_mmu.c
