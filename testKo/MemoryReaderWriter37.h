...
private:
	...
	inline int rwProcMemDriver_Connect(const char * lpszDriverFileNode) {
	...
	inline BOOL rwProcMemDriver_Disconnect(int nDriverLink) {
		if (nDriverLink < 0) { return FALSE; }
		//�Ͽ���������
		close(nDriverLink);
		return TRUE;
	}
	inline BOOL rwProcMemDriver_SetMaxDevFileOpen(int nDriverLink, uint64_t max) {
		if (nDriverLink < 0) { return FALSE; }
		char buf[8] = { 0 };
		*(uint64_t*)&buf[0] = max;
	...
	inline BOOL rwProcMemDriver_HideKernelModule(int nDriverLink) {
		if (nDriverLink < 0) { return FALSE; }
	...
	inline uint64_t rwProcMemDriver_OpenProcess(int nDriverLink, uint64_t pid) {
	...
	inline BOOL rwProcMemDriver_ReadProcessMemory(
		int nDriverLink,
		uint64_t hProcess,
		uint64_t lpBaseAddress,
		void * lpBuffer,
	...
	inline BOOL rwProcMemDriver_ReadProcessMemory_Fast(
		int nDriverLink,
		uint64_t hProcess,
		uint64_t lpBaseAddress,
		void * lpBuffer,
	...
	inline BOOL rwProcMemDriver_WriteProcessMemory(
		int nDriverLink,
		uint64_t hProcess,
		uint64_t lpBaseAddress,
		void * lpBuffer,
	...
	inline BOOL rwProcMemDriver_WriteProcessMemory_Fast(
		int nDriverLink,
		uint64_t hProcess,
		uint64_t lpBaseAddress,
		void * lpBuffer,
	...
	inline BOOL rwProcMemDriver_CloseHandle(int nDriverLink, uint64_t hProcess) {
		if (nDriverLink < 0) { return FALSE; }
		if (!hProcess) { return FALSE; }
	...
	BOOL rwProcMemDriver_VirtualQueryExFull(int nDriverLink, uint64_t hProcess, BOOL showPhy, cvector vOutput, BOOL * bOutListCompleted) {
		if (nDriverLink < 0) { return FALSE; }
		if (!hProcess) { return FALSE; }
		char buf[8] = { 0 };
		*(uint64_t*)&buf[0] = hProcess;
	...
	BOOL rwProcMemDriver_CheckMemAddrIsValid(int nDriverLink, uint64_t hProcess, uint64_t lpBaseAddress) {
		if (nDriverLink < 0) { return FALSE; }
		if (!hProcess) { return FALSE; }
		char ptr_buf[16] = { 0 };
	...
	inline BOOL rwProcMemDriver_GetProcessPidList(int nDriverLink, BOOL bIsSpeedMode, cvector vOutput, BOOL * bOutListCompleted) {
		if (nDriverLink < 0) { return FALSE; }
		uint64_t len = 0;
		int bufSize = 8 + 1 + len;
		char *buf = (char*)malloc(bufSize);
	...
	inline BOOL rwProcMemDriver_GetProcessGroup(int nDriverLink, uint64_t hProcess,
		uint64_t *nOutUID,
		uint64_t *nOutSUID,
		uint64_t *nOutEUID,
		uint64_t *nOutFSUID,
	...
	inline BOOL rwProcMemDriver_SetProcessRoot(int nDriverLink, uint64_t hProcess) {
		if (nDriverLink < 0) { return FALSE; }
		if (!hProcess) { return FALSE; }
	...
	inline BOOL rwProcMemDriver_GetProcessRSS(int nDriverLink, uint64_t hProcess, uint64_t *outRss) {
		if (nDriverLink < 0) { return FALSE; }
		if (!hProcess) { return FALSE; }
		char buf[8] = { 0 };
		*(uint64_t*)&buf[0] = hProcess;
	...
	inline BOOL rwProcMemDriver_GetProcessCmdline(int nDriverLink, uint64_t hProcess, char *lpOutCmdlineBuf, size_t bufSize) {
		if (nDriverLink < 0) { return FALSE; }
		if (!hProcess) { return FALSE; }
