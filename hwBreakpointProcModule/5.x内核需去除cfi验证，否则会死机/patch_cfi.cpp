...
/*
	...
	size_t next_hook_func_addr = patch_do_execve(file_buf, str_root_key, first_hook_func_addr, do_execve_entry_addr, do_execve_key_reg, t_mode_name, v_cred_offset, v_seccomp_offset, vec_patch_bytes_data);
	if (v_hook_func_start_addr.size() > 1) {
		next_hook_func_addr = v_hook_func_start_addr[1];
	}
	if (next_hook_func_addr) {
		next_hook_func_addr = patch_avc_denied(file_buf, next_hook_func_addr, sym.avc_denied_offset, t_mode_name, v_cred_offset, vec_patch_bytes_data);
	}
